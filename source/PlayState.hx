...
class PlayState extends MusicBeatState
{
	...
	var timeBar:FlxBar;
	var timeBarBG:FlxSprite; // found da goofy
	var barRect:FlxSprite;
	...
	override public function create()
	{
		...
		{
			case 'tutorial':
				dialogue = ["Hey ya", 'Today, were teaching how to sign '];
			case 'screwed':
				dialogue = ["FUCK YOOOOOOOOOOOOOOOU!", 'You know what, I WILL BLOCK YOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOU!!!!! YOU BROKE MY PHOOOONE!!'];
		}
		...
		barRect = new FlxSprite(timeBar.x, timeBar.y).makeGraphic(Math.floor(timeBar.width), Math.floor(timeBar.height), FlxColor.TRANSPARENT);
		...
		healthBar.createFilledBar(dad.screwedColor, boyfriend.screwedColor);
		// healthBar
		add(healthBar);
		...
		var credBool:Bool = leCreds != '';
		var textY:Float = healthBar.y + 52;
		if(credBool){
			textY = healthBar.y + 23;
		}

		var songCred = new FlxText(4, healthBar.y + 50,0,leCreds, 16);
		songCred.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		songCred.scrollFactor.set();
		add(songCred);
		songCred.cameras = [camHUD];

		var namez:Array<String> = [
			'Fuck you',
			'Screwed',
			'Screwedfriend',
			'RedstoneSC',
		...
		var kadeEngineWatermark = new FlxText(4, textY,0,SONG.song + ' - '  + CoolUtil.difficultyString(), 16);
		kadeEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		kadeEngineWatermark.scrollFactor.set();
		add(kadeEngineWatermark);
		kadeEngineWatermark.cameras = [camHUD];

		var goofer = new FlxText(0,0,0,'${namez[FlxG.random.int(0,namez.length-1)]} Engine (KE ' + MainMenuState.kadeEngineVer + ")", 16);
		goofer.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		goofer.scrollFactor.set();
		goofer.setPosition(FlxG.width - (goofer.width + 5), 5);
		add(goofer);
		goofer.cameras = [camHUD];
	...
	override public function update(elapsed:Float)
	{
		...
		if (healthBar.percent < 20){
			...
			(iconP2.animation.curAnim.numFrames == 3 ? iconP2.animation.curAnim.curFrame = 2 : iconP2.animation.curAnim.curFrame = 0);
		}
		else if (healthBar.percent > 80){
			iconP2.animation.curAnim.curFrame = 1;
			(iconP1.animation.curAnim.numFrames == 3 ? iconP1.animation.curAnim.curFrame = 2 : iconP1.animation.curAnim.curFrame = 0);
		}
		else{
	...
	override function beatHit()
	{
		...
		if(curBeat % gfSpeed == 0){ // gapple bounce haralambi
			if(curBeat % (gfSpeed * 2) == 0){
				iconP1.scale.set(1.1, 0.8);
				iconP2.scale.set(1.1, 1.8);
				FlxTween.angle(iconP1, -30, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quintOut});
				FlxTween.angle(iconP2, 30, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quintOut});
			}
			else{
			...
			FlxTween.tween(iconP1, {'scale.x': 1, 'scale.y': 1}, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quintOut});
			FlxTween.tween(iconP2, {'scale.x': 1, 'scale.y': 1}, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quintOut});
		} // I love Kade Engine 1.2 no cap
